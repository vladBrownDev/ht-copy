...
Y.namespace('M.atto_table').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _setTable: function(e) {
        ...
        var nl = "\n";
        var tableId = Y.guid();
        var dir = 'dir="' + this.buttons.table.coreDirection + '"';


        // Set direction according to current page language.
        tablehtml = '<br/>' + nl + '<div ' + dir + ' style="' +
            this.buttons.table.styleAlignment + '"><table id="' + tableId + '"' + dir + '>' + nl;

        var captionstyle = '';

        // Set direction to table title.
        var captiondirection = '<span ' + dir + ' style="' +
            this.buttons.table.styleAlignment + '">' + Y.Escape.html(caption.get('value')) + '</span>';

        if (captionposition.get('value')) {
            captionstyle = ' style="caption-side: ' + captionposition.get('value') + '"';
        }
        tablehtml += '<caption' + captionstyle + '>' + captiondirection + '</caption>' + nl;
        i = 0;
        if (headers.get('value') === 'columns' || headers.get('value') === 'both') {
            i = 1;
