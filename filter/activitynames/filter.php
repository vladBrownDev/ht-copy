...
class filter_activitynames extends moodle_text_filter {
     ...
     */
    public function filter($text, $options = []): string {
     ...
     */
    protected function get_cached_activity_list($courseid): array {
        global $USER;
     ...
     * @throws moodle_exception
     */
    protected function get_activity_list($courseid): array {
        $activitylist = [];
        ...
        if (!empty($modinfo->cms)) {
            ...
            foreach ($sortedactivities as $cm) {
                $foundmultilang = false;
                $dom = new DOMDocument;
                $dom->loadHTML('<?xml encoding="utf-8" ?>' . $cm->name);
                $multilangspans = $dom->getElementsByTagName('span');
                foreach ($multilangspans as $span) {
                    if ($span->getAttribute('class') == 'multilang') {
                        $foundmultilang = true;
                        $name = $span->textContent;
                        $title = s(trim(strip_tags($name)));
                        $currentname = trim($name);
                        $entitisedname  = s($currentname);
                        if (!empty($title)) {
                            $hreftagbegin = html_writer::start_tag('a', ['class' => 'autolink',
                                'title' => $title, 'href' => $cm->url]);
                            $idlang = $cm->id.'ml_'.$span->getAttribute('lang');
                            $activitylist[$idlang] = new filterobject($currentname, $hreftagbegin, '</a>', false, true);
                            if ($currentname != $entitisedname) {
                                $activitylist[$cm->id.'-e'] =
                                    new filterobject($entitisedname, $hreftagbegin, '</a>', false, true);
                            }
                        }
                    }
                }
